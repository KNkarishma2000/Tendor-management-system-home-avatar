...
export default function ResidentGallery() {
  ...
  // Pre-check for Upload Button
  const handleOpenModal = () => {
    if (!isApproved) {
      toast.error("Wait for Admin approval! You can share moments once your profile is verified.", {
        icon: '⏳',
        duration: 4000
      });
      return;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isApproved) return toast.error("Verification required to upload.");
    ...
    try {
      ...
      toast.success("Photos submitted! They will appear in the gallery once approved.");
      ...
      {!isApproved && (
        <div className="mb-8 bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-center gap-3 text-amber-800 animate-in slide-in-from-top duration-500 shadow-sm">
          ...
          <div className="text-sm font-medium">
            ...
            <p className="opacity-80">Account verification is required to upload new photos to the community gallery.</p>
          </div>
        </div>
      )}
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
          {photos.length > 0 ? (
            ...
            <div className="col-span-full text-center py-24 bg-white rounded-3xl border border-dashed border-slate-300">
               ...
               <p className="text-slate-400 font-semibold">No community photos yet.</p>
            </div>
          )}
        </div>
      )}
