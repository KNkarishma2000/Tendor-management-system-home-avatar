...
import { tenderAdminAPI, communityAPI,BiddingTenderAPI } from '../../api/auth.service'; 
...
useEffect(() => {
  const loadSupplierData = async () => {
    try {
      ...
      // 1. Fetch Tenders (For the "Tender Board")
      const tendersRes = await tenderAdminAPI.getAllTenders();
      
      // 2. Fetch the logged-in supplier's bids (For stats and sidebar)
      const bidsRes = await BiddingTenderAPI.getAllMyBids(); 

      // Axios data handling
      const allTenders = tendersRes.data?.data || [];
      const myBids = bidsRes.data?.data || []; // This is the 'formattedBids' from your controller

      // Calculate dynamic stats from the fetched bids
     setStats({
  activeTenders: allTenders.filter(t => t.status === 'PUBLISHED').length,
  myBids: myBids.length,
  // Match 'WON' (backend) or 'AWARDED' (common alias)
  wonTenders: myBids.filter(b => b.status === 'WON' || b.status === 'AWARDED').length,
  pendingReviews: myBids.filter(b => b.status === 'SUBMITTED').length
});

      // Update state for lists
      setTenders(allTenders.slice(0, 5));
      setMyRecentBids(myBids.slice(0, 3));
  ...
  EST. BUDGET: ₹{tender.budget_estimate} // Changed from budget_estimation
</div>
                  </div>
                </div>
